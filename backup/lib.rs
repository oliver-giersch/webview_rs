...
//TODO LIST
//TODO: Moving the webview after initialization causes fatal stuff
//REMEDY:
//- put sys::webview into Arc right away (Arc<(webview, Option<ExternalInvokeFn>, )>)
//-
...
impl<T> Webview<T> {
    ...
    pub fn run(&self, blocking: bool) {
        ...
        let webview = self.inner_webview() as *mut _ as usize;
        println!("webview addr before run = 0x{:x}", webview);
    ...
    //TODO: Won't work because the webview struct has to be moved
    #[inline]
